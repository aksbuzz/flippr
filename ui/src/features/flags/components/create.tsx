...
import { z} from 'zod';
import { useCreateFlag, createFlagSchema, type CreateFlagSchema } from '../api/create-flag';

export const CreateFlag = () => {
  const createFlagMutation = useCreateFlag();
  
  const [formState, setFormState] = useState<CreateFlagSchema>({ name: '', key: '', description: '' });
